...
def run():
    ...
    for factor in range(int(-edit_degree), int(edit_degree)):
        global_i = 0
        global_time = []
        out_path_base = os.path.join(edit_directory_path, f'{str(factor)}')
        os.makedirs(edit_directory_path, exist_ok=True)
        out_path_results = os.path.join(out_path_base, 'inference_results')
        out_path_coupled = os.path.join(out_path_base, 'inference_coupled')
        out_path_w = os.path.join(out_path_base, 'inference_w')
        out_path_ww = os.path.join(out_path_base, 'inference_ww')
