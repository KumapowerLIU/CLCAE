...
def run():
    ...
    out_path_input = os.path.join(test_opts.exp_dir, 'inference_input')
    out_path_w = os.path.join(test_opts.exp_dir, 'inference_w')
    out_path_ww = os.path.join(test_opts.exp_dir, 'inference_ww')
    out_path_ww_embedding = os.path.join(test_opts.exp_dir, 'inference_ww_embedding')
    out_path_w_embedding = os.path.join(test_opts.exp_dir, 'inference_w_embedding')
    os.makedirs(out_path_input, exist_ok=True)
    os.makedirs(out_path_ww_embedding, exist_ok=True)
    os.makedirs(out_path_w_embedding, exist_ok=True)
    ...
    for input_batch in tqdm(dataloader):
        ...
        with torch.no_grad():
            ...
            tic = time.time()
            result_batch, results_ww_batch, results_w_batch, latent_base, latent_refine = run_on_batch(input_cuda, image_avg, net,
                                                                                        opts, iter )
            toc = time.time()
        ...
        for i in range(opts.test_batch_size):
            ...
            if opts.couple_outputs or global_i % 100 == 0:
                ...
                im_input_save_path = os.path.join(out_path_input, os.path.basename(im_path))
                Image.fromarray(np.array(input_im.resize(resize_amount))).save(im_input_save_path)
...
def run_on_batch(inputs, image_avg, net, opts, iter, latent_offset=None, factor=None):
    ...
    return result_batch, results_ww, results_w, latent_base, latent_refine
